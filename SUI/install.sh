...
## Setup config.js change your private key
mv config.js config.js.old && touch config.js
sudo tee /root/sui/sui_meta_miner/config.js <<EOF
...
const settings = {
    "local": {
        "phrase": "coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin coin co>
        "packageId": "",
        "firstVPackageId": "",
        "packagePath": "./move",
    },
    "mainnet": {
        "phrase": "suiprivkey", // set it as cli parameter
...
## Buat bash script runing miner
nano start.sh
node mine.js --fomo --meta --chain=mainnet --phrase="suiprivkey"
chmod +x start.sh 

## Runing Miner
nohup ./start.sh >/root/sui/sui_meta_miner/suimon.log 2>&1 &
